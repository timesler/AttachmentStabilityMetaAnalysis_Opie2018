...
data$NotB.NotB.4 <- rowSums(dataNum[, c("A.A.4", "A.C.4", "A.D.4", "C.A.4", "C.C.4", "C.D.4", "D.A.4", "D.C.4", "D.D.4")])
...
file <- "Output/CONT/BB_Results.csv"
fileFig <- "Output/CONT/BB_Results_figure.csv"
write.table(
  "BB Transitions Tables", 
  file = file, 
  row.names = F, 
  col.names = F, 
  sep = ","
...
for (interval in intervals)
{
  ...
  for (class1 in classes)
  {
    ...
    class1.n <- rowSums(data.int[,grep(paste0("^", class1), names(data.int), value = T)])
