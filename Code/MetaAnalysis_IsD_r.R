...
dataRaw$Authors[!as.logical(dataRaw$PriorInclusion_DO)] <- paste0(dataRaw$Authors[!as.logical(dataRaw$PriorInclusion_DO)],"*")
...
Summary.RVE(
  RVEModel,
  fileName = "Output/StatisticalResults_IsD.txt"
)
...
forest.RVE(
  ...
  fileName = "Figures/IsD_Forest.tiff",
  extraCol = "k",
  extraColName = "k"
) 
...
funnel.RVE(
  RVEModel,
  fileName = "Figures/IsD_Funnel.tiff",
  trimnfill = T,
  plotReg = T
)
